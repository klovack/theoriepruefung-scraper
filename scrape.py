from playwright.async_api import async_playwright, Playwright, Page, Locator, BrowserContext
...
def get_file_name(category: str, sub_category: str, resource_src: str, resource_type: ResourceType) -> str:
    return f'{folder_prefix}/{category}/{sub_category}/{resource_type}/{resource_src.split("/")[-1].split("#")[0]}'

async def get_list_of_questions(page: Page, context: BrowserContext, questions_list: list[QuizQuestion], category_name: str, sub_category_name: str):
    ...
    for i, question in enumerate(questions):
        await save_question(context, questions_list, category_name, sub_category_name, question)
    
async def save_question(context, questions_list, category_name, sub_category_name, question):
    ...
    points = (await body_locator.locator("div.points").inner_text()).split(" ")[1]
    ...
    if has_image:
        resource_src = await question_locator.locator("div.image img").get_attribute("src")
        resource_type = "image"
    ...
    questions_list.append(QuizQuestion(question_text, options, correct_options, points, sub_question, QuizQuestionResource(get_file_name(category_name, sub_category_name, resource_src, resource_type), resource_type) if resource_src else None))

    await question_page.close()

async def get_subcategories(page: Page, context: BrowserContext, sub_categories_dict: dict[str, list[QuizQuestion]], category_name: str):
    print(f"get_subcategories in {category_name}")
    sub_categories: list[Locator] = await page.locator("ul.teaser-list-chapters li").all()
    ...
    for sub_category in sub_categories:
        ...
        await get_list_of_questions(sub_category_page, context, sub_categories_dict[sub_category_name], category_name, sub_category_name)
...
async def scrape_autovio(page: Page, context: BrowserContext):
    ...
    for index, category in enumerate(categories):
        ...
        await get_subcategories(category_page, context, all_categories[category_text], category_text)
